...
function run()
    ...
    @ccall "lib/libchess.so".train_model(chess_games.games::Ptr{Ptr{ChessGame}}, chess_games.no_games::Cint, train_info::TrainingInfo)::Cvoid
    no_unique_moves = @ccall "lib/libchess.so".get_no_keys(urgencies::Ptr{HTUrgencies})::Cint
    println("Unique moves: $no_unique_moves")

    # write the urgencies to a file
    @ccall "lib/libchess.so".write_ht_urgencies_to_binary_file("ht_julia_urgencies.bin"::Cstring, urgencies::Ptr{HTUrgencies})::Cvoid;
