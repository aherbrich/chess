    ...
    for(int i = 1; i < (&movelst)->nr_elem+1; i++){
        ...
        if (move->from == from && move->to == to){
            ...
            if(move->flags & 0b1000 && strlen(move_str) == 5){
                if((move_str[4] == 'q' || move_str[4] == 'Q') && (move->flags == QPROM || move->flags == QCPROM)){
                    move_t *copy = copy_move(move);
                    free_pq(&movelst);
                    return copy;
                }
                else if((move_str[4] == 'r' || move_str[4] == 'R') && (move->flags == RPROM || move->flags == RCPROM)){
                    move_t *copy = copy_move(move);
                    free_pq(&movelst);
                    return copy;
                }
                else if((move_str[4] == 'b' || move_str[4] == 'B') && (move->flags == BPROM || move->flags == BCPROM)){
                    move_t *copy = copy_move(move);
                    free_pq(&movelst);
                    return copy;
                }
                else if((move_str[4] == 'n' || move_str[4] == 'N') && (move->flags == KPROM || move->flags == KCPROM)){
                    move_t *copy = copy_move(move);
                    free_pq(&movelst);
                    return copy;
                }
...
void *main_interface_loop(void *args) {
    ...
    while (1) {
        ...
        // GO command
            ...
            pthread_create(&game_thread, NULL, start_search, (void *) searchdata);
        } 
        // NEWGAME command
