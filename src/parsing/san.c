...
int is_piece(char character) {
    return (character == 66 || character == 75 || character == 78 ||
            character == 81 || character == 82);
}
...
move_t* str_to_move(board_t* board, char* token) {
    ...
    /* if piece move */
        ...
        if (piece == 'N') {
            ...
            move = find_bishop_move(board, file1, rank1, file2, rank2,
                                    single_ambigious, double_ambigious);
        } else if (piece == 'R') {
            move = find_rook_move(board, file1, rank1, file2, rank2,
                                  single_ambigious, double_ambigious);
        } else if (piece == 'Q') {
            move = find_queen_move(board, file1, rank1, file2, rank2,
                                   single_ambigious, double_ambigious);
        } else {
            move = find_king_move(board, file1, rank1, file2, rank2,
                                  single_ambigious, double_ambigious);
        }
    }
    /* if castle move */
