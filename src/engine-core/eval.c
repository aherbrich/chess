...
int gamephaseInc[14] = {0,1,1,2,4,0,0,0,0,1,1,2,4,0};
int mg_table[14][64];
int eg_table[14][64];
...
void initialize_eval_tables(void) {
    for (int p = PAWN; p <= KING;  p++) {
        for (int sq = 0; sq < 64; sq++) {
            ...
            mg_table[p+0b1000][sq] = mg_value[p] + mg_pesto_table[p][FLIP(sq)];
            eg_table[p+0b1000][sq] = eg_value[p] + eg_pesto_table[p][FLIP(sq)];
        }
    }
}
...
int eval_board(board_t* board) {
    ...
    /* evaluate each piece */
    for (int sq = 0; sq < 64; ++sq) {
        piece_t pc = board->playingfield[sq];
        if (pc != NO_PIECE) {
    ...
    /* tapered eval */
    int mgScore = mg[board->player] - mg[SWITCHSIDES(board->player)];
    int egScore = eg[board->player] - eg[SWITCHSIDES(board->player)];
